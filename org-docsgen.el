;;; org-docsgen.el --- Generate org API documentation from Emacs Lisp source files -*- lexical-binding: t; -*-

;; Author: sam kleinman <sam@tychoish.com>
;; Maintainer: sam kleinman <sam@tychoish.com>
;; Version: 0.1.0
;; Package-Requires: ((emacs "29.1") (xtdlib "0.1.0") (compat "30.0.0.0"))
    ...
    (lambda (f)
      (let ((base (file-name-nondirectory f)))
        (or (string-match-p "/test/" f)
            (string-match-p "-test\\.el$\\|\\`test-.*\\.el$\\|-pkg\\.el$\\|-autoloads\\.el$" base))))
...
(defun org-docsgen--src-block-elisp-p ()
  "Return non-nil when point is inside an emacs-lisp source block."
  (let ((info (ignore-errors (org-babel-get-src-block-info t))))
    (and info (equal (nth 0 info) "emacs-lisp"))))
...
`org-docsgen-run' src block."
  (with-current-buffer (or buffer (current-buffer))
    (save-excursion
      (goto-char (point-min))
      (let (found)
        (while (and (not found) (re-search-forward "(\\s-*org-docsgen-run\\b" nil t))
          (when (org-docsgen--src-block-elisp-p)
            (setq found t)))
        found))))
      ...
      (let (found)
        (while (and (not found) (re-search-forward "(\\s-*org-docsgen-run\\b" nil t))
          (when (org-docsgen--src-block-elisp-p)
            (setq found t)))
        (unless found
          (user-error "org-docsgen: no `org-docsgen-run' emacs-lisp src block in %s" file)))
